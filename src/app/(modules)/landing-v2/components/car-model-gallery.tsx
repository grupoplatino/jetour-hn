...
export default function CarModelsGallery({ title = 'Nuestros Modelos', models, className }: CarModelsGalleryProps) {
  return (
        ...
        {models.map((model, index) => (
          ...
          >
            ...
            <h3 className="text-2xl md:text-3xl font-bold text-center relative">
              {model.name}
              <span className="block h-0.5 w-0 bg-orange-500 group-hover:w-full transition-all duration-300 mt-1"></span>
            </h3>
          </Link>
        ))}
      </div>
