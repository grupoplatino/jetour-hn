...
export default function LandingPage() {
  ...
  const CarPlaceholder = ({ image, customClass }: { image: string; customClass: string }) => {
    return <Image src={image} width={400} height={400} alt="Car Placeholder" className={`absolute ${customClass}`} style={translateCenter} />;
  };
  ...
  return (
    <>
      ...
      <HeroSection image={carData.hero.backgroundImage} title={carData.hero.title} subtitle={carData.hero.subTitle} />

      <BenchMarkSection>
        <CarPlaceholder image={carData.carModels[0]} customClass="top-[60%] z-10 left-[50%] h-[25rem] w-[45rem]" />
        <CarPlaceholder image={carData.carModels[1]} customClass="h-[16rem] w-[30rem] top-[40%] left-[15%]" />
        {carData.carModels[2] ? <CarPlaceholder image={carData.carModels[2]} customClass="h-[16rem] w-[30rem] top-[40%] left-[85%]" /> : <></>}
      ...
      {carData.wideImages.map((sectionData: any, index: any) => {
        ...
        return (
          <ColorContext.Provider key={index} value={carData.primaryColor}>
            <WideImage key={index} title={sectionData.title} image={sectionData.image} />
          </ColorContext.Provider>
        );
      })}
      ...
      {carData.cropedWideImages.map((sectionData: any, index: any) => {
        ...
        return (
          <ColorContext.Provider key={index} value={carData.primaryColor}>
            <SlicedWideImage title={sectionData.title} images={sectionData.images}>
